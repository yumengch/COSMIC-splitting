...
def advection(initialProfile=topHat, xmin = 0, xmax = 1, nx = 100, nt = 125, dt = 0.2):
    ...
    dx = np.zeros([nx+1])
    # J = np.zeros_like(dx)
    dx[1:-1] = 0.5*(x1[1:-1] - x1[:-2]) + 0.5*(x1[2:] - x1[1:-1])
    dx[0] = 0.5*(x1[1] - x1[0]) + 0.5*(x1[-1] - x1[-2])
    dx[-1] = dx[0]

    # initial conditions
    u = np.zeros(nx+1)
    u[:] = 0.122																					# velocity (u) change at this line!!!!
    ...
    # 1D PPM
    phi = COSMIC(phiOld.copy(), x1, u, dt, nt,J)
    ...
    plt.clf()
    plt.plot(x,phi)
    plt.plot(x,phiExact, 'r')

    plt.ylim(-0.1, 1.1)
    plt.show()


advection(initialProfile=cosBell, xmin = 0, xmax = 1., nx = 100, nt = 100,  dt = 2)