...
#---------------------------------------------------------------------------------
    ...
    for t in xrange(int(nt)):
        ...
        #---------------------------
        idx_x, r_x, c = departure_x(X_edge, xmin, xmax, u, dx, dt, Lx)
        # print np.max(a-c)
        #----------------------------------------------------------
        ...
        #----------------------------------------
        print 'at ', t,' time step, the maximum of phi is ',np.max(phiOld), np.min(phiOld)
        # if t%100 == 0:
        #     plt.figure(1)
        #     plt.clf()
        #     plt.plot(X_cntr,phi)
...
def departure_x(X, xmin, xmax, u, dx, dt, L):
    ...
    c = (X - x_depart)/dx
    c = np.where( (u>0) & (c <0), (xmax - x_depart + X)/dx, c )
    c = np.where( (u<0) & (c >0), -(xmax + x_depart - X)/dx, c )
    # print c, 
    # print x_depart
    N = c.astype(int)
...
#     #     else:
#     #         dmphi[j] = 0
#     # if (phiOld[0]-phiOld[-1])*(phiOld[0]-phiOld[-2]) > 0:
#     #     dphi = 0.5*(phiOld[0]-phiOld[-2])
#     #     dmphi[-1] = min(np.abs(dphi),2*np.abs(phiOld[-1]-phiOld[-2])\
#     #             ,2*np.abs(phiOld[0]-phiOld[-1]))*np.sign(dphi)
#     # else:
#     #     dmphi[j] = 0
...
#             phi_mid[i] = phiOld[idx[i+1]] + (1 - c_r[i+1])*dmphi[idx[i+1]]/2.
#             # print phi_mid[i]
#         else:
#             # print c_r[i+1] < 0.0
#             phi_mid[i] = phiOld[idx[i+1]] - (1 + c_r[i+1])*dmphi[idx[i+1]]/2.
