import numpy as np
def COSMIC(phiOld, cx, cy, u, v, X_cntr, X_edge, Y_edge, Y, dt, nt, J, initialProfile, mesh, change):
#---------------------------------------------------------------------------------
# Author: Yumeng Chen
...
def departure_y(X_cntr, X_edge, Y_edge, Y, ymin, ymax, v, dy, dt, Lx, Ly, mesh):
    ...
    #-----------------------
    if mesh == 'quad':
        y_depart = phys_to_compt_SB(y_depart, Ly, f_quad(X_cntr,Lx, Ly))
    elif mesh == 'V':
        y_depart = phys_to_compt_SB(y_depart, Ly, f_V(X_cntr,Lx, Ly))
    else:
        y_depart = y_depart
