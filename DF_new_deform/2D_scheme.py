...
def COSMIC(phiOld,nt,epsilon,dx,dy,J,xmin,ymin,dt,cx,cy,distorted):
    ...
    for t in xrange(int(nt)):
        psi = deformational(x,y,xmin,ymin,nx,ny,Lx,Ly,nt,dt, distorted,t,change = True)
        ...
        if (t+1)*dt == 4.:
            phiT4 = phiOld.copy()
        print t,np.max(phiOld[:-1,:-1])
 
    return phiT1,phiT2,phiT3,phiT4,phi
