import numpy as np
def COSMIC(phiOld,cx,cy,nt,epsilon,dx,dy):
    ...
    for t in xrange(int(nt)):
        ...
        for j in xrange(ny):
            ...
            OUT = flux(nx,dx,cx[j,:],phi_mid[j,:-1],mass[j,:])
            XC_AY[j,:-1] = conf(nx,cx[j,:],OUT)
        XC_AY[-1,:] = XC_AY[0,:]
        XC_AY[:,-1] = XC_AY[:,0]

        #the COSMIC splitting update
        phi = phiOld+ 0.5*(XC+XC_AY)+0.5*(YC+YC_AX)
        phi[-1,:] = phi[0,:]
        phi[:,-1] = phi[:,0]
...
def conservative(phiOld,cx,cy,nt,epsilon,dx,dy):
    #Calculate the number of grid points
    nx = len(phiOld[0,:]) - 1
    ny = len(phiOld[:,0]) - 1
    ...
    for t in xrange(int(nt)):
        ...
        for i in xrange(nx):
            phi_mid[:,i],mass[:,i] = PPM(phi_CX[:,i],cy[:,i],ny,epsilon,dy)
            OUT = flux(ny,dy,cy[:,i],phi_mid[:-1,i],mass[:,i])
            YC_CX[:-1,i] = conf(ny,cy[:,i],OUT)
            YC_CX[-1,i] = YC_CX[0,i]
        YC_CX[:,-1] = YC_CX[:,0]
...
def flux(nx,dx,c,phi_mid,mass):
    ...
    for i in xrange(nx):
        ...
        if c[i+1]>0:
            if i>k:
                OUT[i] = (phi_mid[i]*dc[i+1]*dx+mass[i]-mass[k])/dx
            elif i==k:
                OUT[i] = phi_mid[i]*dc[i+1]*dx/dx
            else:
                OUT[i] = (phi_mid[i]*dc[i+1]*dx+mass[i]+mass[-1]-mass[k])/dx
        elif c[i+1]<0:
            if i>k:
                OUT[i] = (-phi_mid[i]*dc[i+1]*dx+mass[-1]-mass[i]+mass[k])/dx
            elif i ==k:
                OUT[i] = -phi_mid[i]*dc[i+1]*dx/dx
            elif i<k:
                OUT[i] = (-phi_mid[i]*dc[i+1]*dx-mass[i]+mass[k])/dx
