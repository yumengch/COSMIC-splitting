...
def COSMIC(phiOld,nt,epsilon,dx,dy,J,initialProfile,xmin,ymin,dt,UchangewithT,cx,cy,r,r1,r2):
    # plt.ion()
    ...
    for t in xrange(int(nt)):
        ...
        phi[:,-1] = phi[:,-2] = phi[:,-3] = phi[:,-4] = phi[:,-5] = 0
        phi[:,0] = phi[:,1]= phi[:,2] = phi[:,3] = phi[:,4] = 0
        phi[-1,:] = phi[-2,:] = phi[-3,:] = phi[-4,:] = phi[-5,:] = 0
        phi[0,:] = phi[1,:] = phi[2,:] = phi[3,:] = phi[4,:] = 0
        # phi = np.where(r>r1, 0, phi)
...
def PPM(phiOld,c,nx,epsilon,dx,eta1=20 ,eta2 = 0.05):
    ...
    # phi_mid[-2] = phi_mid[0]
    # phi_mid[-1] = phi_mid[1]
    phi_mid[-1] = phi_mid[-2] = 0
    phi_mid[0] = phi_mid[1] = 0
