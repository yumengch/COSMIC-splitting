...
def contours_ter(x,X,Y,phiCOS,phiOld,phiCOSMid,phiExact,phiMidExact,dx,dt,dy):
    ...
    plt.figure(1,figsize=(17.75, 10.25))
    plt.clf()
    ax = plt.subplot(111)
    # fig = 
    ...
    plt.xlim([-75,75])
    plt.ylim([0,20])
    plt.figtext(.5, 0.08, "min = "+str(np.round(np.min(phiCOS-phiExact),3))+', max = '+str(np.round(np.max(phiCOS-phiExact),3)),horizontalalignment='center',
